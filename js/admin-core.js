...
document.addEventListener("DOMContentLoaded", () => {
  ...
  textarea.addEventListener("input", () => {
    ...
    const currentMode = document.querySelector(
      'input[name="previewMode"]:checked'
    ).value;
  ...
  if (quickDeleteBtn && deleteUrlInput) {
    quickDeleteBtn.onclick = async () => {
      ...
      const tweetId =
        extractTweetId(input) || (input.match(/^\d{10,}$/) ? input : null);
  ...
  // quick Add Btn 点击后从拷贝板获取链接并添加到 textarea 中，支持批量添加
  const quickAddBtn = document.getElementById("quickAddBtn");
  // 从拷贝板获取链接（优先使用 Clipboard API，失败时回退到 prompt）
  async function readClipboardText() {
    ...
    // 回退：弹窗让用户粘贴
    try {
      return window.prompt("Paste Twitter URL(s):") || "";
    } catch {
      return "";
    }
  }
  ...
  if (quickAddBtn) {
    quickAddBtn.onclick = async () => {
      ...
      localStorage.setItem(IMPORT_CACHE_KEY, newText);
      updatePreview(extractUrls(newText));
      addUrlInput.value = "";
      showStatus(`Added ${urls.length} URL(s) to the list`, "success");
    };
  }
  ...
  window.deleteUrl = async (url) => {
    ...
    inputs.forEach((input) => {
      const card =
        input.closest(".tweet-card-wrapper") ||
        input.closest(".image-mode-card");
    ...
    try {
      ...
      if (data.ok) {
        ...
        cards.forEach((card) => {
          ...
          const btn =
            card.querySelector(".card-delete-btn") ||
  ...
  publishBtn.addEventListener("click", async () => {
    ...
    console.log(
      "Calling saveDailySnapshot with date:",
      d,
      "urls:",
      selectedUrls.length
    );
    ...
    if (result && result.ok) {
      ...
      if (result.totalUrls !== undefined) {
        ...
        if (result.newUrls !== undefined && result.newUrls < result.totalUrls) {
          message += `（新增 ${result.newUrls} 条，${
            result.totalUrls - result.newUrls
          } 条为去重后的已存在链接）`;
        }
      } else {
...
async function updatePreview(urls) {
  ...
  if (urls.length === 0) {
    list.innerHTML =
      '<div style="text-align: center; padding: 4rem; color: var(--muted-foreground); font-family: var(--font-mono); font-size: 0.875rem; text-transform: uppercase; letter-spacing: 0.1em;">No links detected yet</div>';
    return;
  }
        ...
        .then((data) => {
          ...
          const findFirstImage = () => {
            if (Array.isArray(data.media_extended)) {
              const imgMedia = data.media_extended.find(
        ...
        .catch((err) => {
          ...
          if (placeholder)
            placeholder.innerHTML =
              '<span style="color: var(--text-primary); font-size: 0.8rem;">Failed to load info</span>';
        });
    }
  });
}
...
function toggleItemStyle(checkbox) {
  // Find parent wrapper
  const wrapper =
    checkbox.closest(".tweet-card-wrapper") ||
    checkbox.closest(".image-mode-card");
  if (!wrapper) return;
...
function fillLoadingQueue() {
  const unloadedContainers = Array.from(
    document.querySelectorAll(
      ".tweet-embed-container:not(.loaded):not(.loading)"
    )
  ...
  window.twttr.widgets
    ...
    .then((el) => {
      ...
      if (el) {
        ...
        showRetryButton(
          container,
          'Tweet not found or unavailable<br><small style="opacity: 0.7;">可能原因：推文已删除、账号被封禁、或视频有地区限制</small>'
        );
    ...
    .catch((err) => {
      ...
      showRetryButton(
        container,
        'Failed to load tweet<br><small style="opacity: 0.7;">网络错误或Twitter API问题</small>'
      );

...
let apiBaseUrl = 'https://ttmouse.com/';
...
function buildApiUrl(path) {
    if (!apiBaseUrl) {
        ...
        if (!host.includes('localhost') &&  !host.includes('www.xiliy.com') && !host.includes('vercel.app')) {
            // EXCEPTION: Keep tweet_info on Vercel to use distributed Serverless IPs (avoid 429)
            if (path.includes('tweet_info')) {
                return path;
            }
            console.log('Hybrid Mode detected: Defaulting API to https://www.xiliy.com');
            return `https://www.xiliy.com/${path.replace(/^\/+/, '')}`;
        }
        return path;
    }

    // CRITICAL FIX: Force relative paths (HTTPS) for production domain
    if (window.location.hostname === 'www.xiliy.com' || window.location.hostname.endsWith('.www.xiliy.com')) {
        apiBaseUrl = '';
    ...
    if (window.location.protocol === 'https:') {
        ...
        if (window.location.hostname.includes('www.xiliy.com')) {
    ...
    if (path.startsWith('http')) {
        ...
        if (window.location.protocol === 'https:' && path.startsWith('http:')) {
            ...
            if (path.includes('38.55.192.139')) {
                return path.replace('http:', 'https:').replace('38.55.192.139', 'www.xiliy.com');
            }
